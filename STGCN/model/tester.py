...
def model_test(inputs, batch_size, n_his, n_pred, inf_mode, load_path, id=1):
    '''
    ...
    with open('results/stgcn.csv', "a") as myfile:
        myfile.write(
            "forecast_horizon, my_seed, mean_mse, mean_rmse, mean_mae, mean_mre\n")
        for a in range(len(tmp_idx)):
            #te = evl[ix - 2:ix + 1]
            #print(f'Time Step {ix + 1}: MAPE {te[0]:7.3%}; MAE  {te[1]:4.3f}; RMSE {te[2]:6.3f}.')
            #[MSE(v, v_), RMSE(v, v_), MAE(v, v_), MRE(v, v_)])
            #print(f'{ix + 1}; {id}; {te[0]:7.3%}; {te[1]:4.3f}; {te[2]:6.3f}; {te[3]:6.3f}\n')
            my_str = f'{a + 1}, {id}, {evl[0 + 4 * a]}, {evl[1 + 4 * a]}, {evl[2 + 4 * a]}, {evl[3 + 4 * a]}\n'
            myfile.write(my_str)
            print(my_str)
