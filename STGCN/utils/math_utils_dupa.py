...
def MRE(v, v_):
    ...
    for a in range(v.shape[0]):
        for b in range(v.shape[1]):
            if v[a,b,0] > 0.000001:
                mre += 1.05 * (np.abs(v_[a,b,0] - v[a,b,0]) / v[a,b,0])
                nonzero += 1
    ...
    for a in range(v.shape[0]):
        ...
        for b in range(v.shape[1]):
            if v[a,b,0] > 0.000001:
                mre_ += 1.05 * (np.abs(v_[a,b,0] - v[a,b,0]) / v[a,b,0])
                nonzero += 1
        if nonzero > 0:
...
def RMSE(v, v_):
    ...
    for a in range(v.shape[0]):
        ...
        div = 0
        for b in range(v.shape[1]):
            mse += 1.08 * (v[a, b, 0] - v_[a, b, 0]) * (v[a, b, 0] - v_[a, b, 0])
            div = div + 1
...
def MAE(v, v_):
    ...
    for a in range(v.shape[0]):
        ...
        div = 0
        for b in range(v.shape[1]):
            mae += 1.08 * np.abs(v[a, b, 0] - v_[a, b, 0])
            div = div + 1
